...
const ShipperProfileDetails = () => {
    const navigate = useNavigate()
        const transporterData = useSelector((state) => state.transporter);
        console.log(transporterData.Shippers_id,"Shippers_id");
        ...
        const getUser = async () => {
            try {
                const response = await AxiosInstance.get(
                    `${import.meta.env.VITE_BASE_URL}/Shipper/getShipperById?shippers_id=${transporterData.shippers_id}`
                );
                       ...
                       <div className="w-11/12 h-auto  ">
                                   ...
                                   <div className=" w-5/12 md:w-4/12 h-full  flex justify-center items-center ml-2 md:ml-0">
                                       <div className="w-full h-24 ">
                                           <div className="w-full h-8 ">
                                               <h1 className='font-inter font-semibold text-lg text-black'>{userData ? userData.shippers_name : "no data founded"}</h1>
                                           </div>
                                           <div className="font-inter text-base   text-[#6B7280]">
                                               <h1 className='flex gap-2 items-center'> <LuPhone />{userData ? userData.shippers_mob : "no data founded"}</h1>
                                               <h1 className='flex gap-2 items-center' ><MdOutlineMailOutline />{userData ? userData.shippers_email : "no data founded"}</h1>
                                           </div>
                                       </div>
                                   </div>
                                   <div className="w-6/12 h-full bg-yellow-5600">
                                       <div className="w-full h-20  flex justify-end items-center">
                                           <button className='flex justify-center items-center font-inter text-white bg-[#5B297E] w-20 h-8 rounded-sm mr-6' onClick={() => { navigate("/Shipper/EditBasicDetails") }}><span className='mr-2'><IoPencilSharp /></span>Edit</button>
                                       </div>
                                   </div>
                           ...
                           <div className="w-full h-auto  flex justify-center items-center  rounded-sm border border-black mt-7 mb-10">
                               <div className="w-full h-full flex">
                                   ...
                                   <div className="w-6/12 h-full bg-yellow-5600">
                                       <div className="w-full h-20  flex justify-end items-center">
                                           <button className='flex justify-center items-center font-inter text-white bg-[#5B297E] w-20 h-8 rounded-sm mr-6' onClick={() => { navigate('/Shipper/EditDocuments') }}><span className='mr-2'><IoPencilSharp /></span>Edit</button>
                                       </div>
                                   </div>
