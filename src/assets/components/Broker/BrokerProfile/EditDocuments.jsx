...
const EditDocuments = () => {
    ...
    const handleSubmit = async(event) => {
        ...
        try {
         ...
         const Aadaharurl = await uploadFileToS3(AadharCard, AadharCard.name);
                    const GSTCertificateurl = await uploadFileToS3(GSTCertificate, GSTCertificate.name);
            
                    // Store uploaded URLs in state
                    setAadharurl(Aadaharurl);
                    seGSTCertificateUrl(GSTCertificateurl);
            
                    // Prepare payload for backend API
                    const payload = {
                        brokers_id: transporterData.borkers_id,
                        brokers_docs: [
                            { name: "Aadhaar Card", type_id: 20, image: Aadharurl },
                            { name: "GST Certificate", type_id: 22, image: GSTCertificateUrl }
                        ]
                    };
                     const response = await AxiosInstance.post('/Broker/insertBrokerProfileDoc', payload,);
